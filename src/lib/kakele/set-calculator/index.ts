import type { KakeleEquipmentItems, KakeleWeaponItems } from '@/src/types/index';
...
export function getBestItemsForLevel({
  ...
  const filteredItems =
    (items.filter(
      (item) =>
        ...
        !lockedSlots.some((slot) => slot.slot === item.slot)
    ) as KakeleEquipmentItems[]) || [];

  const result: Record<string, KakeleSetItem> = {};

  const equipmentSlots = ['Necklace', 'Helmet', 'Ring', 'Primary Hand', 'Armor', 'Legs', 'Boots', 'Tool'];
  ...
  if (energyCount >= 5) {
    ...
    for (const key of Object.keys(result)) {
      ...
      const bestItem = desirableItems.find((item) => item.slot === currentItem.slot && item.name !== currentItem.name);

      if (bestItem && bestItem.stats[attribute] > currentItem.stats[attribute]) {
        newEnergyItems.push({
    ...
    for (let index = 0; index < newEnergyItems.length; index++) {
      ...
      result[newEnergyItems[index].item.slot.toLowerCase().replace(' ', '_')] = newEnergyItems[index].item;
      count -= 1;
    }
  }
  ...
  for (const slot of equipmentSlots) {
    if (!result[slot.toLowerCase().replace(' ', '_')]) {
      const anyItem = desirableItems
        .filter((item) => equipmentSlots.includes(item.slot) && item.slot === slot)
