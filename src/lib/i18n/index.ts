...
const getLanguageFromHeaders = (reqHeaders: Headers) => {
  const nextUrlHeader = reqHeaders.has('next-url') && reqHeaders.get('next-url');
  const hasLanguageInHeader = nextUrlHeader && nextUrlHeader.indexOf(`"lng":"`) > -1;

  if (!hasLanguageInHeader) {
    return null;
  }

  const qsObj = JSON.parse(nextUrlHeader.substring(nextUrlHeader.indexOf('{'), nextUrlHeader.indexOf(`}`) + 1)) as {
    lng: string;
  };
...
export function getLanguageFromCookie(requestCookies: RequestCookies | ReadonlyRequestCookies) {
  return acceptLanguage.get(requestCookies.get(cookieName)?.value) as Language | null;
}

export function getLanguageFromAcceptHeader(requestHeaders: Headers) {
  return acceptLanguage.get(requestHeaders.get('Accept-Language')) as Language | null;
}
...
export function detectLanguage() {
  ...
  const language = languageFromCookie || languageFromHeaders || languageFromAcceptHeader || fallbackLng;
