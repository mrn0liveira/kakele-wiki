			...
			<header className="z-50 h-16 w-full">
				<nav className="flex w-full items-center justify-between px-8 py-8 lg:py-4">
					...
					>
						...
						<div
							className={cn(
								"flex flex-row flex-nowrap font-pixAntiqua font-bold text-white",
							)}
						>
					...
					<div className="hidden w-full flex-row items-center justify-end lg:flex">
										...
										<ul className="grid w-[500px] grid-cols-2 gap-3 p-1">
											...
											<div className="grid grid-cols-2 items-center justify-center gap-2 p-2">
												{itemOptions.map((item) => (
													<Link
														className="flex h-full w-full flex-col items-center justify-center rounded-lg border-1 border-b-2 border-white/10 bg-white/5 p-1 hover:bg-white/10"
														href={`/items?filter=${item.id}`}
														key={item.id}
													>
														<Image
															alt={item.label}
														...
														<span className="text-md ml-2">
															{t(
																`kakele.itemTypes.${item.id.replaceAll(
																	" ",
																	"",
																)}`,
															)}
											...
											>
												<div className="flex h-fit w-fit flex-col items-start justify-start">
													<div className="h-fit w-full justify-end">
														...
														<div className="z-50 m-2 flex flex-col drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]">
															<span className="text-2xl font-bold">
																{t("navbar.tools.title")}
															</span>
															<span className="text-xs">
																{t("navbar.tools.subhead")}
															</span>
														</div>
													</div>
												</div>
						...
						<Link href={session.data?.user ? "/dashboard" : "/login"}>
							<button
								className="items-center justify-center gap-2 animate-buttonheartbeat flex flex-row rounded-md bg-orange-800/40 px-4 py-1 text-sm font-semibold text-white"
								type="button"
							>
								{session.data?.user ? (
									<Settings2 size={16} />
							...
							<div className="flex h-full flex-col">
								<div className="flex justify-between">
									...
									>
										...
										>
											Kakele{" "}
											<span className="font-bold text-orange-500">Wiki</span>
										</div>
									</Link>
									<p
										...
										>
											<div className="flex flex-col gap-2">
												<div className="relative py-2">
													<h2 className="__className_d6dd13 text-3xl uppercase">
														{t("navbar.items.title")}
													</h2>
													<div
														...
														style={{
															background:
																"linear-gradient(to right, rgba(245,93,39,0.8) 0%, rgba(0, 0, 0, 0) 100%)",
														}}
													/>
												</div>
												...
												>
													{itemOptions.map((option) => (
														...
														>
															<Link href={`/items?filter=${option.id}`}>
