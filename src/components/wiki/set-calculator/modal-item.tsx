              ...
              <div className='relative flex h-full w-full flex-col items-center justify-center rounded-md p-2 px-8'>
                <div className='absolute -top-48 w-fit text-nowrap text-center text-3xl font-bold'>
                  <span className='text-xs font-bold'>
                    <span className={cn(getEnergyTextColor(item.energy))}>{item.energy}</span>{' '}
                    <span className={cn(getRarityTextColor(item.rarity))}>{item.rarity}</span> <span>{item.type}</span>
                  </span>
                  <h3 className='text-md text-center font-bold lg:text-lg'>{item.name}</h3>
                </div>
                <Image
                  alt={item.name}
                  ...
                  width={256}
                />
                <div className='absolute -top-8 flex items-center justify-center gap-8'>
                ...
                <div className='mt-28 flex flex-wrap items-center justify-center gap-4'>
                  <div className='relative flex items-center justify-center rounded-md bg-stone-900/50 p-2 px-4'>
                    {Intl.NumberFormat().format(item.level)}
                    <span className='absolute -top-2 text-xs'>Level</span>
                  </div>
                  <div className='relative flex items-center justify-center rounded-md bg-stone-900/50 p-2 px-4'>
                    {Intl.NumberFormat().format(item.value)}
                    <span className='absolute -top-2 text-xs'>Value</span>
                  </div>
                </div>
              </div>
            </motion.div>
