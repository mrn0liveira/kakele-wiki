...
function getItemSourcesByMonsterName(name: string | undefined, defaultName: string | undefined) {
  return monsters.filter((item) => item.loot.includes(String(name)) && item.name !== defaultName) ?? [];
}

function getTaskTotalXpAndGold(task: KakeleTaskItem | null, amount: number, value: number) {
        ...
        >
          ...
          <div className='z-[60] flex flex-col items-center justify-end rounded-lg bg-transparent'>
              ...
              <div className='relative flex h-full w-full flex-col items-center justify-center rounded-md p-2 px-8'>
                ...
                {item && (
                  <div className='relative flex flex-row'>
                    <span className='px-1 text-xs'>{task?.amount}</span>
                ...
                <div className='flex flex-row items-center gap-4'>
                  <div className='flex flex-col gap-4 p-4'>
                    <div className='flex flex-col'>
                      <h3 className='text-md font-bold text-zinc-300'>{t('taskCalculator.valueInputTitle')}</h3>
                      <span className='text-xs'>{t('taskCalculator.valueInputSubtitle')}</span>
                    </div>
                    <Input
                  ...
                  <div className='flex flex-col items-center justify-center gap-4 p-4'>
                    <div className='flex flex-col'>
                      <h3 className='text-md font-bold text-zinc-300'>{t('taskCalculator.amountInputTitle')}</h3>
                      <span className='text-xs'>{t('taskCalculator.amountInputSubtitle')}</span>
                    </div>
                    <Input
                      className='flex w-[8rem] items-center justify-center border-none text-center text-xl font-bold'
                      maxLength={12}
                ...
                <div className='flex flex-col items-center justify-center gap-4 lg:flex-row'>
                  <div className='flex max-w-[16rem] flex-wrap justify-center gap-2'>
                  ...
                  {taskData && (
                    <div className='lg:text-md flex flex-col items-start justify-center gap-2 text-xs'>
                      ...
                      <span>
                        <span className='rounded-md bg-green-900 px-4 font-bold'>{t('taskCalculator.xp')}</span>{' '}
                        <span className='rounded-md bg-green-950 px-4'>
                          {new Intl.NumberFormat().format(taskData.xp)}
                        </span>
                      </span>
                      <span>
                        <span className='rounded-md bg-yellow-900 px-4 font-bold'>{t('taskCalculator.gold')}</span>{' '}
                      ...
                      {taskData.loss < 0 && taskData.completed >= 1 && (
                        <span>
                          <span className='rounded-md bg-red-900 px-4 font-bold'>{t('taskCalculator.loss')}</span>{' '}
                          <span className='rounded-md bg-red-950 px-4'>
                            {Number.isNaN(taskData.loss) ? '0' : new Intl.NumberFormat().format(taskData.loss)}
                          </span>{' '}
                          <span className='rounded-md bg-red-950/50 px-4'>
                            {Number.isNaN((taskData.loss / taskData.gold) * 100)
                              ? '0'
                              : new Intl.NumberFormat().format(Math.floor((taskData.loss / taskData.gold) * 100))}{' '}
                            %
                          </span>
                        </span>
                      )}
                      {taskData.loss > 0 && taskData.completed >= 1 && (
                        <span>
                          <span className='rounded-md bg-violet-900 px-4 font-bold'>{t('taskCalculator.profit')}</span>{' '}
            ...
            <motion.div
              ...
              className='absolute top-4 flex flex-col items-center justify-center gap-4 p-4'
              initial={{
                opacity: 0,
                y: -100,
              }}
            ...
            >
              <Link
                className='flex w-[70vw] items-center justify-center rounded-xl bg-orange-800/80 bg-stripe-gradient md:h-6 lg:w-[40rem]'
                href='https://coinshub.com.br/affiliate/biridim/'
              >
                <h3 className='md:text-md p-1 text-xs'>
                  <span className='font-black text-orange-300'>CoinsHub</span>{' '}
                  <span className='font-sans text-orange-400'>{t('coinsHub.ad1')}</span>
                </h3>
              </Link>
            </motion.div>
          </div>
