...
import type { KakeleEquipmentItems, KakeleEquipmentOrWeapon, KakeleWeaponItems } from '@/types';
      ...
      {item ? (
        <BlessItemContainer>
          <div className='flex h-full w-[18rem] flex-col justify-center gap-2'>
            ...
            />
            <div className='relative flex h-fit w-[18rem] flex-col items-start justify-center rounded-xl border-[1px] border-zinc-800 bg-zinc-900 p-4 md:hidden'>
              {warnMessage &&
                [
          ...
          <div className='relative mt-8 flex h-fit flex-col items-center justify-center gap-2 md:mt-0'>
            ...
            <div className='relative h-fit w-[18rem] rounded-xl border-[1px] border-zinc-700 bg-zinc-800 p-4 md:w-[22rem]'>
                    ...
                    {blessItems.map((item: KakeleEquipmentItems | KakeleWeaponItems, index: number) => {
                      return (
                        <motion.div
                          ...
                          className='relative flex flex-row items-center rounded-md border-[1px] border-zinc-600 bg-zinc-700 p-2 drop-shadow-md hover:border-zinc-400'
                          exit={{ opacity: 0, y: 30 }}
          ...
          <div className='flex flex-col gap-4'>
            <div className='relative hidden h-fit w-[18rem] flex-col items-center justify-center rounded-xl border-[1px] border-zinc-800 bg-zinc-900 p-4 md:flex'>
              {warnMessage &&
                [
                  'blessError.initialValueGreaterThanFinalValue',
                  'blessError.initialAndFinalValuesMustBeNumbers',
...
const BlessItemHeader = React.forwardRef(
  ({
    ...
    return (
      <div className='card-shine-effect-2 border-zinc flex w-full flex-col items-center justify-center rounded-lg border-[1px] border-zinc-800/70 bg-zinc-900 p-4'>
        ...
        <Image
          ...
          className='h-40 w-40'
          src={
            new URL(`https://res.cloudinary.com/dl3asnoii/image/upload/v1713726404/sprites/items/${item.name}.png`).href
          }
        />
      </div>
    );
...
const BlessItemInfo = React.forwardRef(
  ({
    ...
    return (
      <div className='w-full rounded-xl border-[1px] border-zinc-700 bg-zinc-800'>
        {item.level && (
          <div className='flex flex-row items-center justify-between space-x-12 p-2 text-xs'>
            <h3 className='font-bold text-zinc-400'>{levelText}</h3>
            <h4 className='rounded-md bg-zinc-900/70 p-[2px] px-4'>{Intl.NumberFormat().format(item.level)}</h4>
          </div>
        )}
        {item.value && (
          <div className='flex flex-row items-center justify-between space-x-12 p-2 text-xs'>
            <h3 className='font-bold text-zinc-400'>{valueText}</h3>
            <h4 className='rounded-md bg-zinc-900/70 p-[2px] px-4'>{Intl.NumberFormat().format(item.value)}</h4>
          </div>
        )}
        {item.energy && (
          <div className='flex flex-row items-center justify-between space-x-12 p-2 text-xs'>
            <h3 className='font-bold text-zinc-400'>{energyText}</h3>
            <h4 className='rounded-md bg-zinc-900/70 p-[2px] px-4'>{t(`kakele.energy.${item.energy}`)}</h4>
          </div>
        )}
      </div>
    );
      ...
      <>
        <div className='flex flex-col gap-1'>
          ...
          <div className='ml-8 flex flex-col items-start text-sm'>
            <span className='flex flex-row items-center gap-2 text-xs text-zinc-300'>
            ...
            <span className='mt-2 rounded-md bg-zinc-950 p-[2px] px-2 text-xs text-orange-500'>
              {totalBonusText}{' '}
