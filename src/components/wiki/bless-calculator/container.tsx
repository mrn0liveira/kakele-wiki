...
import type {
	KakeleEquipmentItems,
	KakeleEquipmentOrWeapon,
	KakeleWeaponItems,
} from "@/src/types";
...
export default function BlessCalculatorContainer({
	lng = "en",
}: { lng: string }) {
	...
	const [queryIgnore, setQueryIgnore] = useQueryState(
		"ignore",
	...
	const [initialValue, setInitialValue] = useState<number>(
		Number(queryFirstBless) || 0,
	);
	const [finalValue, setFinalValue] = useState<number>(
		Number(querySecondBless) || 6,
	);
	...
	useEffect(() => {
		const item = kakeleItems.find(
			(x: any) => x.name.toLowerCase() === value?.value.toLowerCase(),
		);
	...
	useEffect(() => {
		const item = kakeleItems.find(
			(x) =>
				x.id === Number(queryItem) &&
				(x.type === "Equipment" || x.type === "Weapon"),
						...
						<div className="relative flex h-fit w-[18rem] flex-col items-start justify-center rounded-xl border-[1px] border-stone-800 bg-stone-900 p-4 md:hidden">
							...
							<div className="items-cenre flex flex-col text-start">
								<h2 className="text-xl font-bold">
									{t("blessCalculator.optionsTitle")}
								</h2>
								<span className="text-sm">
									{t("blessCalculator.optionsDescription")}
								</span>
							</div>
						...
						<BlessItemStats
							blessBonus={Math.floor(
								(((item.stats.attack < 0 ? 0 : item.stats.attack) + attack) *
									//@ts-ignore
									((BlessPercentages[item.rarity]?.[finalValue] as number) ||
										0)) /
									100,
							)}
					...
					<div className="relative mt-8 flex h-fit flex-col items-center justify-center gap-2 md:mt-0">
						...
						<div className="relative h-fit w-[18rem] rounded-xl border-[1px] border-stone-700 bg-stone-800 p-4 md:w-[22rem]">
							...
							<div className="mb-4 flex flex-col items-start text-start">
								<h2 className="text-xl font-bold">
									{t("blessCalculator.requiredItemsTitle")}
								</h2>
								<span className="text-sm">
									{t("blessCalculator.requiredItemsDescription")}
								</span>
							</div>
												...
												return (
													...
													>
														<Image
															...
															src={
																new URL(
																	`https://raw.githubusercontent.com/mrn0liveira/kakele-biridim/main/src/assets/sprites/items/${item.name}.png`,
																).href
															}
															width={64}
														/>
							...
							<div className="mt-4 flex h-[10rem] max-h-[10rem] w-[18rem] flex-col gap-2 text-sm">
								...
								<div className="no-scrollbar flex flex-col gap-2 overflow-y-auto">
									{ignoredItems?.map((item: string, index: number) => {
										const ignoredItem = kakeleItems.find(
										...
										return (
											<div
												...
												onClick={() => {
													setQueryIgnore(
														ignoredItems.filter((i) => i !== item),
													);
													setIgnoredItems(
														ignoredItems.filter(
															(ignoredItem: string) => ignoredItem !== item,
														),
													);
												}}
												onKeyDown={() => {
...
const BlessItemContainer = React.forwardRef(
		...
		return (
			<div className="card-shine-effect-2 border-stone flex w-full flex-col items-center justify-center rounded-lg border-[1px] border-stone-800/70 bg-stone-900 p-4">
				<span className="text-xs">
					<span className={cn(getItemRarityColor(item.rarity), "font-black")}>
						{rarityName}
					</span>{" "}
					{itemType}
				</span>
			...
			<div className="w-full rounded-xl border-[1px] border-stone-700 bg-stone-800">
				{item.level && (
					<div className="flex flex-row items-center justify-between space-x-12 p-2 text-xs">
						...
						<h4 className="rounded-md bg-stone-900/70 p-[2px] px-4">
...
const BlessItemInfoGold = React.forwardRef(
						...
						<span className="mt-2 rounded-md bg-stone-950 p-[2px] px-2 text-xs text-orange-500">
							...
							<span className="font-bold">
								{Intl.NumberFormat().format(
									(value < 0 ? 0 : value) + itemStat + blessBonus,
								)}
							</span>{" "}
						</span>
					</div>
